...
function BooksSinglePage() {
  ...
  const GetBook = async () => {
    const data = await axios.get('http://localhost:5000/book/search?book=' + id.slice(1), {
      headers: {
        ...
        <BooksSingleWrapperContent>
          {
            card.map((el) => (
              <>
                <BooksSingleWrapperAuthor src={'http://localhost:5000/' + el.image} width='505' height='681' alt="Imgauthor" />
        ...
        <BooksSingleBooksList>
          {
            cards.map((element) => (
              <>
               ...
               <NavLink to={'/BooksSinglePage'+element.genre_id+element.title }>
                   <BooksSingleBooksImgs src={'http://localhost:5000/' + element.image} width='190' height='283' alt="bookCards" />
